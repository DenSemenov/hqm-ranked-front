...
import { AudioProvider } from 'shared/Garland/AudioManager';
import Garland from 'shared/Garland/Garland';
...
const Header = () => {
  ...
  const loginButton = useMemo(() => {
    if (!isMobile) {
      if (userName && currentUser && isAuth) {
        return (
          <Badge
            count={currentUser.isBanned ? 'BAN' : undefined}
            offset={[-16, 16]}
          >
            <Avatar
              shape='square'
              style={{ cursor: 'pointer', borderRadius: 16 }}
              src={storageUrl + 'images/' + currentUser.id + '.png' + '?t=' + query}
    ...
    return (
      <List
        ...
        renderItem={(item, index) => {
          return (
            <Row className={styles.patrolHeader}>
              <Col span={12}>
                <TeamItem
                  id={item.teamId}
                  name={item.teamName}
              ...
              >
                <Space size={'small'}>
                  <Button
                    type='primary'
                    icon={<CheckOutlined />}
                    onClick={() => onApplyInvite(item.inviteId)}
                  />
                  <Button
                    type='primary'
                    danger
  ...
  const playerInvitesNotify = useMemo(() => {
    if (playerInvites.length !== 0 && !teamsState.team) {
      return (
        <Popover
          content={playerInvitesContent}
          title='Invites to team'
          trigger={'click'}
        >
                      ...
                      label: (
                        <>
                          ...
                          <Text
                            ...
                            type='secondary'
                          >
                            {new Date(x.dateStart).toLocaleDateString() + ' - ' + new Date(x.dateEnd).toLocaleDateString()}
                          </Text>
                        </>
                      ),
                    };
  ...
  const getContractItem = (item: IContractResponse) => {
    ...
    return (
      <Badge.Ribbon
        text={current}
        placement='end'
        style={{ top: 'calc(-54px + 100%)', display: item.isSelected ? undefined : 'none' }}
        key={item.id}
      >
        <div
          ...
          style={{
            ...
            filter: item.isHidden ? 'grayscale()' : undefined,
            outline: item.isSelected ? '4px solid #1c40af' : undefined,
          }}
          onClick={() => onSelectContract(item)}
        >
          <div
            className={styles.contactCardPoints}
            style={{ bottom: item.isSelected ? 32 : 8 }}
          >
            <FaCoins color={'gold'} />
            {item.points}
          </div>
          {item.isSelected && <Tag className={styles.contactCardDate}>{leftDays + 'd left'}</Tag>}
          ...
          <div style={{ display: 'flex', gap: 16, flexDirection: 'column' }}>
            <Row>
              ...
              <Col
                span={6}
                className='right-align'
                style={{ display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'end' }}
              >
                <FaCoins color={'gold'} />
                {coins}
              </Col>
  ...
  return (
    <>
      <div style={{ display: 'flex', gap: 4, alignItems: 'center', cursor: 'pointer', zIndex: 1 }}>
        <div
          className={styles.rankedLogo + ' ' + (currentMode !== IInstanceType.Ranked ? styles.filteredLogo : undefined)}
          onClick={() => onChangeMode(IInstanceType.Ranked)}
        >
          RNKD
        </div>
        <svg
        ...
        <div
          className={styles.teamsLogo + ' ' + (currentMode !== IInstanceType.Teams ? styles.filteredLogo : undefined)}
          onClick={() => onChangeMode(IInstanceType.Teams)}
        >
          TEAMS
        </div>
        {endsIn}
